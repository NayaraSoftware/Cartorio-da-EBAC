...
int registro() //Função responsavel por cadastrar os usuarios no sistema
{
	...
	file = fopen(arquivo, "a");// "a" significa se o arquivo não existir ele cria um novo
	fprintf(file,",");
	fclose(file);
...
{	
	...
	FILE *file;
	file = fopen(cpf,"r"); //o arquivo deve ser aberto para leitura "r" significa ler
...
int main()
{
	...
	for(laco=1;laco=1;)
	{
		...
		switch(opcao)
		{
			...
			break; // encerramento da instrução
			...
			default: //quando a propriedade nao esta presente
